# Workspace unit (dev). Talks to the accounts API (mws) but not into a workspace,
# so it includes root + databricks-mws (no workspace provider). Reads networking
# outputs via a dependency block (replaces the old terraform_remote_state).
